...
def convert_pptx_to_scorm(pptx_file, output_folder, title, author, use_slide_titles, use_mini_images, style="black", fullscreen=False):
    try:
        ...
        slide_titles = [slide.shapes.title.text if slide.shapes.title else f"Slide {i + 1}"
                        for i, slide in enumerate(Presentation(pptx_file).slides)] if use_slide_titles else None
...
def generate_html_files(slide_images, output_folder, title, author, slide_titles, use_mini_images, style="black", fullscreen=False):
    try:
        for i, image in enumerate(slide_images):
            if use_mini_images:
                thumbnails = "".join([
                    f'<div style="text-align: center; margin-bottom: 10px;"><a href="slide_{j + 1}.html"><img src="{os.path.basename(img)}" alt="Slide {j + 1}" style="width:100%; background-color: #ccc; {"border: 6px solid red; box-shadow: 0 0 10px 2px rgba(255, 0, 0, 0.5);" if i == j else "border: 2px solid transparent;"}" id="thumb_{j + 1}"></a><br><span style="color: {"white" if style in ["black", "black-red", "green", "white", "green-white"] else "black"};">{slide_titles[j] if slide_titles else j + 1}</span></div>'
                    for j, img in enumerate(slide_images)])
            else:
                thumbnails = "".join([
                    f'<div style="text-align: center; margin-bottom: 10px;"><a href="slide_{j + 1}.html"><span style="color: {"white" if style in ["black", "black-red", "green", "white", "green-white"] else "black"};" id="thumb_{j + 1}">{slide_titles[j] if slide_titles else j + 1}</span></a></div>'
                    for j, img in enumerate(slide_images)])
            ...
            if style == "black":
                ...
                text_color = "black"
                title_color = "white"  # Ensure the title is visible in white style
            elif style == "green":
                ...
                text_color = "white"
                title_color = "white"
            elif style == "green-white":
                background_color = "linear-gradient(to bottom, #013220, white)"  # Bottle green to white gradient
                text_color = "black"
                title_color = "white"
            ...
            <head>
                ...
                <style>
                    ...
                    .sidebar span {{
                        ...
                        padding: 10px;
                        font-size: 14px;  /* slightly reduced */
                        color: {"white" if style in ["green", "green-white"] else text_color};  /* Ensure white color for green and green-white styles */
                    }}
                    .sidebar span:hover {{
                        background-color: transparent;
                    }}
...
class SCORMConverterApp:
    def __init__(self, root):
        ...
        # Set custom fonts
        self.title_font = font.Font(family="Helvetica", size=26, weight="bold")  # increased
    ...
    def create_widgets(self):
        ...
        self.style_combobox = ttk.Combobox(style_frame, values=["black", "white", "green", "black-red", "green-white"], font=self.label_font)
        ...
        self.footer_label = tk.Label(self.root, text="ver. 1.2", font=self.label_font, bg="lightblue")
    ...
    def convert(self):
        ...
        try:
            ...
            if "Access is denied" in str(e):
                messagebox.showerror("Błąd", "Brak uprawnień do zapisu pliku. Uruchom aplikację jako administrator.")
            else:
