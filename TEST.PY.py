...
class SCORMConverterApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Konwerter PPSX na SCORM")
    ...
    def create_widgets(self):
        tk.Button(self.root, text="Wybierz plik PPSX", command=self.select_ppsx_file).pack(pady=10)
        self.ppsx_label = tk.Label(self.root, text="Nie wybrano pliku")
        self.ppsx_label.pack(pady=5)

        tk.Button(self.root, text="Wybierz folder wyjściowy", command=self.select_output_folder).pack(pady=10)
        self.output_label = tk.Label(self.root, text="Nie wybrano folderu")
        self.output_label.pack(pady=5)

        tk.Button(self.root, text="Konwertuj", command=self.convert).pack(pady=20)
    ...
    def convert(self):
        ...
        try:
            ...
            # Convert PPTX to SCORM
            convert_pptx_to_scorm(self.pptx_file, self.output_folder)
